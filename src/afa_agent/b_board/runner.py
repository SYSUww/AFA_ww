...
执行器会按 unit 自动处理百分数：金额÷带 % 的变量时会把百分数转为比率；带 % 的变量÷另一个带 % 的变量会约去百分数单位；金额×带 % 的变量也会自动除以 100。
因此不得再创建 100% 基数、不得在上述运算前后额外乘除 100。pct_change 已直接返回百分数，ratio 使用 percent 格式输出时也会自动乘 100，二者同样不得再手工乘 100。
